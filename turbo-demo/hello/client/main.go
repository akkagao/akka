...
func main() {
	...
	defer cancel()
	r, err := helloClient.SayHello(ctx, &pb.SayHelloRequest{YourName: name})
	if err != nil {
		log.Fatalf("could not greet: %v", err)
	}
	log.Printf("Greeting: %s", r.Message)

	response, err := worldClient.SayWorld(ctx, &pb.SayWorldRequest{YourWold: "test", Password: "Password"})
	if err != nil {
